#include <stdint.h>

class AES {
 private:
  static constexpr uint8_t S_BOX[256] = {
      
  };

  static constexpr uint8_t INVERSE_S_BOX[256] = {
       };

  static constexpr uint8_t R_CON[11] = {
      0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
  };

  static constexpr uint8_t Nb = 4;
  uint8_t Nk;
  uint8_t Nr;

  typedef uint8_t State[4][Nb];
  uint8_t *Key;
  uint8_t *RoundKey;

  static void AddRoundKey(uint8_t round, State* s, uint8_t* roundKey);
  void KeyExpansion(uint8_t* roundKey, uint8_t* key);
  static void RotWord(uint8_t *word);
  static void SubWord(uint8_t *word);
  static void SubBytes(State* s);
  static void ShiftRows(State* s);
  static uint8_t xtime(uint8_t x);
  static void MixColumns(State* s);
  static uint8_t Multiply(uint8_t a, uint8_t b);
  static void InvMixColumns(State* s);
  static void InvSubBytes(State* s);
  static void InvShiftRows(State* s);
  void Cipher(State* s, uint8_t* RoundKey);
  void InvCipher(State* s, uint8_t* RoundKey);

 public:
  AES(uint8_t *key, uint8_t keyLen) : Key(key) {
    if (keyLen == 16) {
      RoundKey = new uint8_t[176];
      Nk = 4;
      Nr = 10;
    } else if (keyLen == 32) {
      RoundKey = new uint8_t[240];
      Nk = 8;
      Nr = 14;
    }
    KeyExpansion(RoundKey, key);
  }
  void encryptECB(uint8_t* buf);
  void decryptECB(uint8_t* buf);

};